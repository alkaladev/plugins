    ...
    async messageRun({ message, args }) {
        ...
        if (input === "estado") {
            ...
            if (isNaN(strikes) || Number.parseInt(strikes) < 1) {
                return message.reply("Las advertencias deben ser un número válido mayor que 0");
            }
            response = await setStrikes(settings, strikes, message.guild);
        } else if (input === "accion") {
            const action = args[1].toUpperCase();
            if (!action || !["TIMEOUT", "KICK", "BAN"].includes(action))
                return message.reply("Acción inválida. La acción debe ser `Timeout`/`Kick`/`Ban`");
            response = await setAction(settings, message.guild, action);
        } else if (input === "debug") {
            const status = args[1].toLowerCase();
            if (!["on", "off"].includes(status))
                return message.reply("Estado inválido `on/off`");
        ...
        // whitelist add
            ...
            const match = message.guild.findMatchingChannels(args[1]);
            if (!match.length) return message.reply(`No se encontró ningún canal que coincida con ${args[1]}`);
    ...
    async interactionRun({ interaction }) {
        ...
        if (sub === "estado") {
            ...
            response = await setAction(
                settings,
                interaction.guild,
                interaction.options.getString("accion"),
            );
        } else if (sub === "debug") {
